...
class OpenAIResponsesModel(Model):
    ...
    def _log_user_input(self, input: str | list[TResponseInputItem]) -> None:
        """Log user input to JSONL recorder when enabled."""
    ...
    def _extract_assistant_log_parts(self, response: Response) -> tuple[str | None, list[dict] | None]:
        """
        Extract assistant text and function calls from Responses API output.
        """
    ...
    def _record_response(
        ...
        """
        Record assistant/tool outputs and the request/response pair in JSONL.
        """
        if self.logger is None:
            return

        assistant_text, tool_calls = self._extract_assistant_log_parts(response)
        if tool_calls:
            self.logger.log_assistant_message(None, tool_calls)
        if assistant_text:
            self.logger.log_assistant_message(assistant_text)

        converted_tools = Converter.convert_tools(tools, handoffs)
        create_params = {
            "model": str(self.model),
            "messages": ItemHelpers.input_to_new_input_list(input),
            "stream": stream,
            "tools": converted_tools.tools,
            "tool_choice": model_settings.tool_choice,
        }
        self.logger.rec_training_data(create_params, response, None, self.agent_name)
